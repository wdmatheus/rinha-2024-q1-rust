...
( 
	...
	inout cliente_id_resp integer default null,
	inout limite_resp integer default null,
	inout saldo_resp integer default null,
    inout transacao_foi_criada boolean default false
)
language plpgsql
as $proc$
begin
    ...
    from
        public.clientes c
        into cliente_id_resp
    where
        c.id = cliente_id;
    
    if cliente_id_resp is null then
        select
            0, 0, 0, false
        into cliente_id_resp, limite_resp, saldo_resp, transacao_foi_criada;
        return;
    end if;
    ...
    update public.clientes
        ...
        into limite_resp, saldo_resp;
    
    if limite_resp is null then
        select
            cliente_id_resp, 0, 0, false
        into cliente_id_resp, limite_resp, saldo_resp, transacao_foi_criada;
        return;
    end if;
    ...
    select
        cliente_id_resp,limite_resp, saldo_resp, true
        into cliente_id_resp, limite_resp, saldo_resp, transacao_foi_criada;
end
$proc$;
